...
use crate::state::SIGNER;

// version info for migration info
const CONTRACT_NAME: &str = "crates.io:contract-account";
...
pub fn instantiate(
    ...
    let singer_addr = deps.api.addr_validate(&msg.signer_addr)?;

    SIGNER.save(deps.storage, &singer_addr)?;

    let allowance = proto::Any {
        type_url: "/cosmos.feegrant.v1beta1.BasicAllowance".to_string(),
        value: proto::BasicAllowance {
            spend_limit: vec![],
            expiration: None,
        }
        .to_bytes()
    ...
    let msg_grant_allowance = proto::MsgGrantAllowance {
        granter: env.contract.address.to_string(),
        grantee: msg.signer_addr,
    ...
    Ok(Response::new()
        .add_message(CosmosMsg::Stargate {
            ...
            value: Binary(msg_grant_allowance.to_bytes().unwrap()),
        })
